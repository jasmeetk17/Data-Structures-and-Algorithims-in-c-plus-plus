...
int main() {
    ...
    std::cout<<"enter the position and data to be inserted before the given position"<<"\n";
    std::cin>>pos>>data;

    // shifting the element by traversing array to the one step before to given position
    for (i=n;i>=pos-1;i--)
    {
        // add the previous index of data to next index for shifting until we reach the before of the given position where we want to insert
        a[i+1]=a[i];
    }
    // add element before to the given position
    a[pos-1]=data;
